...
# need it before pytest can even run: gh-app-token.sh (to resolve the
...
# there verbatim. Must happen before `uv run` below: that command implicitly
# runs `uv sync`, which is what actually resolves synthlib.
APP_KEY_FILE=$(mktemp)
